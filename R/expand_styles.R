...
get_dim_ranges <- function(df_x) {
  ...
  df_style_hashed <- df_x |>
    ...
    dplyr::ungroup() |>
    tidyr::unnest_legacy() |>
    dplyr::mutate(hash = as.integer(factor(hash)))

  ## Based on style merge rows -------
  df_aggregated <- df_x |>
    dplyr::bind_cols(df_style_hashed) |>
    dplyr::arrange(col_id, row_id) |>
    dplyr::group_by(col_id) |>
    dplyr::mutate(line = cumsum(dplyr::coalesce(dplyr::lag(hash),0L) - hash != 0)) |>
    dplyr::group_by(col_id, line) |>
    dplyr::mutate(row_to = max(row_id)) |>
  ...
  df_aggregated <- df_aggregated |>
    dplyr::mutate(dims = paste0(openxlsx2::int2col(col_id), row_id, ":",
                                openxlsx2::int2col(col_id), row_to))
